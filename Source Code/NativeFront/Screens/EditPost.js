...
function EditPost({ route, navigation, props }){
  ...
  function deleteimg(link) {
    const storage = getStorage(firebase.getApp());
    const pointr = ref(storage, link);
    deleteObject(pointr).then(() => {
      console.log("Deleted")
    }).catch((error) => {
      console.log(error)
    });
  ...
  function editpost() {
    ...
    if(image != route.params.link) {
    const id = route.params.id
    submitPost(id)
    deleteimg(route.params.link)}
    setError("")
    ...
    fetch(`${domain}/api/auth/posts/${route.params.id}`, {
      ...
      .then(json => {
        ...
        navigation.navigate("ListingPage", route.params)
      })
      .catch(error => {
        console.log(error)
  ...
  return (
    ...
    <View>
        ...
        {route.params.link != "" && image === null ? <View><Image source={{ uri: route.params.link }} style={{ width: 400, height: 300 }}/><Text></Text></View> : <Text></Text>} 
